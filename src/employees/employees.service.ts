...
export class EmployeesService {
    ...
    async findNeededEmployees(allowed: string[], employees: string[]): Promise<EmployeeEntity[]> {
        ...
        employeeEntities.forEach((employee) => {
            if (!allowed.includes(employee.post)) {
                throw new BadRequestException(`employee with id:${employee.id} haven't needed post`);
            }
        })
